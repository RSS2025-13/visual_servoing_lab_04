...
class ParkingController(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info("Parking Controller Initialized square1")
    ...
    def relative_cone_callback(self, msg):
        ...
        delta_e = distance_error - self.previous_error

        if distance_error != 0 or cone_angle != 0:
            cone_angle = (1 if distance_error > 0 else -1) * self.kp_angle * cone_angle
            car_speed = (self.kp_drive * distance_error + self.kd_drive*delta_e) if np.absolute(distance_error) > .05 else 0
        ...
        car_speed = np.clip(car_speed, -1.0, 1.0) if np.absolute(car_speed) >= .5 else 0.0
